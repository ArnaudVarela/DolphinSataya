...
type Item = { label: string; Icon: (props: SVGProps<SVGSVGElement>) => ReactElement };

const IconSwimmer = (p: SVGProps<SVGSVGElement>) => (
...
const IconDiploma = (p: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" aria-hidden="true" {...p}>
  ...
  return (
    <section className="bg-[#FAF7F2] border-b border-[#EADCC8]">
      <div className="mx-auto max-w-[1200px] px-6 py-8">
        <ul className="grid grid-cols-2 place-items-center gap-y-6 md:grid-cols-4 md:gap-y-0">
          {items.map(({ label, Icon }) => (
            <li key={label} className="relative flex items-center gap-3 md:justify-center">
              <span
                className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#F0E6D6] ring-1 ring-[#EADCC8] text-[#0B3A5B]"
                aria-hidden
              >
